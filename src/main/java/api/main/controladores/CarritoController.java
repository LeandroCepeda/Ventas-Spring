...
public class CarritoController {
	...
	public String interfazCarrito(Model model, HttpServletRequest request) {
		...
		for (ProductoParaVender producto : carrito) total += producto.getTotal();
	...
	public String agregarAlCarrito(@ModelAttribute Producto producto, HttpServletRequest request, RedirectAttributes redirectAttrs) {
		...
		for (ProductoParaVender productoParaVender : carrito) {
			if(productoParaVender.getCantidad() >= productoBuscadoPorCodigo.getExistencia())
				redirectAttrs
                .addFlashAttribute("mensaje", "No puede agregar mas unidades del producto " + productoParaVender.getNombre())
                .addFlashAttribute("clase", "warning");
        return "redirect:/carrito/";
		}
		
		boolean encontradoEnCarrito = false;
	    ...
	    if(!encontradoEnCarrito) {
	    	carrito.add(new ProductoParaVender(productoBuscadoPorCodigo.getNombre(), productoBuscadoPorCodigo.getCodigo(), productoBuscadoPorCodigo.getPrecio(), productoBuscadoPorCodigo.getExistencia(), productoBuscadoPorCodigo.getId(), 1));
	    }
    ...
    public String terminarVenta(HttpServletRequest request, RedirectAttributes redirectAttrs) {
        ArrayList<ProductoParaVender> carrito = this.obtenerCarrito(request);
        // Si no hay carrito o esta vacío, regresamos inmediatamente
        ...
        for (ProductoParaVender productoParaVender : carrito) {
            ...
            ProductoVendido productoVendido = new ProductoVendido(productoParaVender.getCantidad(), productoParaVender.getPrecio(), productoParaVender.getNombre(), productoParaVender.getCodigo(), venta);
            // Y lo guardamos
